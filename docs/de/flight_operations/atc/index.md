---
title: IFR-ATC-Kommunikation in X-Plane
description: "Vollständiger IFR-ATC-Kommunikationsleitfaden für X-Plane — sieben Kapitel von Freigabe über Start, Reiseflug und Anflug bis zur Landung."
---
# ATC

Sieben Kapitel führen durch die ATC-Kommunikation einer vollständigen IFR-Flugphase: von der Flugfreigabe (Clearance) über Pushback, Start, Abflug und Streckenflug bis zu Anflug und Landung. Jedes Kapitel enthält die Standard-Phraseologie, die relevanten Frequenzen und die Informationsquellen aus Charts und FMGS.

- **[Clearance](clearance.md)** — Flugfreigabe und IFR-Clearance
- **[Pushback & Taxi](pushback_taxi.md)** — Pushback und Rollen zur Startbahn
- **[Start](takeoff.md)** — Line Up und Abflug
- **[Abflug & Steigflug](departure.md)** — SID folgen und Steigflug
- **[Streckenflug](enroute.md)** — Reiseflug und Frequenzwechsel
- **[Anflug](approach.md)** — Sinkflug und Positionierung
- **[Landung & Abstellen](landing.md)** — Landefreigabe und Taxi zum Gate
