---
description: "XPME (X-Plane Map Enhancement) streamt Satellitenbilder über ein eingehängtes virtuelles Dateisystem in X-Plane 12. Linux-Einrichtung, kostenlose Stufe gegen kostenpflichtiges Pro-Abo und Vergleich mit AutoOrtho und XEarthLayer."
---
...
!!! warning "Pro ist ein Abonnement, kein Kauf"
    Das Bestellformular unter `k.aiflygo.com/purchase` weist die Lizenz für X-Plane 12 mit **5 $ für 30 Tage** und **40 $ für 365 Tage** aus — das Formular baut seine Preisanzeige dynamisch auf, die aktuellen Beträge sind also dort vor dem Bezahlen zu prüfen. Eine dauerhafte Lizenz gibt es nicht. Eine Lizenz gilt für genau einen PC, ist an die Hardware gebunden (ein Wechsel von CPU oder Datenträger sowie eine Neuinstallation des Systems können sie ungültig machen), und die kommerzielle Nutzung — Flugschulen, Trainingszentren, jeder gewerbliche Betrieb — ist ausdrücklich untersagt. Die Zahlung läuft über PayPal oder Buy Me a Coffee; der Anbieter nennt eine Rückgabefrist von 7 Tagen.
...
Anders als AutoOrtho und XEarthLayer liest XPME das Gelände-[Mesh](../../glossary.md#mesh) nicht aus der Szenerie von X-Plane. Es liefert **Basispakete** mit — regionale [DSF](../../glossary.md#dsf-distribution-scenery-format)- und Terrain-Daten, erzeugt mit einem angepassten [Ortho4XP](../../glossary.md#ortho4xp)-Fork auf ZL16 — die dieselbe Rolle einnehmen wie die DSF/TER-Pakete bei XEarthLayer. Die Basispakete lassen sich nicht von Hand installieren: Sie liegen auf Cloudflare und werden vom eingebauten Downloader geholt, der `aria2c` als Übertragungs-Backend nutzt und die gewählten Pakete nacheinander installiert. Wer hinter restriktiven Netzen sitzt, wird auf einen HTTP-Proxy oder ein VPN verwiesen.
...
Nach dem ersten Start in den Einstellungen den **Pfad für die Basispakete** auf eine schnelle SSD legen und dann in der Ansicht „Downloader" die zu befliegenden Regionen auswählen und installieren. Die Downloads laufen nacheinander und können groß werden. Die Anwendung muss laufen und vor X-Plane gestartet sein, damit das virtuelle Dateisystem eingehängt ist; beim Beenden gilt die umgekehrte Reihenfolge — erst X-Plane schließen, dann in XPME auf „Stop" drücken, sonst wird der Einhängepunkt nicht sauber abgeräumt.
...
- **Unabhängige Linux-Erfahrungsberichte waren für diese Seite nicht überprüfbar.** Der einschlägige Community-Thread liegt in einem Forum, das automatisierte Abrufe blockt, deshalb stützt sich hier nichts darauf.
...
|---|---|---|---|
| Quellcode | Geschlossen | Offen (GPL) | Offen |
...
- **Nutzer mehrerer Simulatoren**: XPME ist das einzige der drei, das X-Plane und Microsoft Flight Simulator mit einer Produktfamilie und einer Oberfläche abdeckt — sein deutlichster struktureller Vorteil.
