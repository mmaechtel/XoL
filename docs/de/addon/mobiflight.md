...
Steht kein zweiter Rechner zur Verfügung, kann eine Windows-VM (KVM/QEMU) auf dem Linux-Host den Connector ausführen. Die Arduino-Hardware wird per USB-Passthrough an die VM durchgereicht (siehe [Docker & Virtualisierung](../docker.md) für KVM-Grundlagen).
