...
- **Compatible with:** A319, A320 CEO/NEO, A321 CEO/NEO, A330-900, A340-600
...
All FlyWithLua scripts are copied to `Resources/plugins/FlyWithLua/Scripts/` unless noted otherwise. The X-Airbus Library goes into `Modules/`. Most scripts ship with sound folders that must reside in the same directory.
...
There are no known Linux-specific issues with the FlyWithLua scripts. FlyWithLua itself must be correctly installed — see [FlyWithLua](flywithlua.md) for dependencies and known issues on Debian.
